...
string newTempID(){
	return IDNode::genVarName(); // 临时变量名
}
...
string generateTAC(Node* root){
	...
	case Node_Type::node_opt:
		...
		//child = child->getBrother();
		if (right_child != nullptr && right_child->getNodeType() == Node_Type::node_opt){ // 由于历史原因..前缀自增节点多了个id本身做哥哥的节点..这里就是拿自己取代哥哥
		...
		tempVar = new IDNode(temp.c_str(), -1, ((ValueNode*)root)->getValueType()); // 变量类型
		tempVar->setValue((ValueNode*)root); // 临时变量的值就是当前表达式的值(runtime的话就为nullptr)
